...
class SemiSegmentationModel(DetectionModel):
    def __init__(self, cfg="yolov8n-seg.yaml", ch=3, nc=None, verbose=True):
        """Initialize YOLOv8 segmentation model with given config and parameters."""
        super().__init__(cfg=cfg, ch=ch, nc=nc, verbose=verbose)
    ...
    def init_criterion(self):
        """Initialize the loss criterion for the SegmentationModel."""
        return v8SemiSegmentationLoss(self)
    ...
    def forward(self, x, *args, **kwargs):#分别调用model和teacher_model的forward方法
        if isinstance(x, dict):  # for cases of training and validating while training.
            if x['is_label'] == True:#输入有标签数据的话获取loss
                return self.loss(x, *args, **kwargs)
            else:#输入无标签数据的话返回预测结果，分别返回教师模型和学生模型的结果后在外面计算loss
    ...
    def pseudo_label(self, batch, preds=None):
        ...
        # if getattr(self, "criterion", None) is None:
        #     self.semi_criterion = self.init_semi_criterion()
