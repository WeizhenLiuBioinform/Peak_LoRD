...
class v8SemiSegmentationLoss(v8DetectionLoss):
    """Criterion class for computing training losses."""
    ...
    def __call__(self, preds, batch):
        ...
        batch_size, _, mask_h, mask_w = proto.shape  # batch size, number of masks, mask height, mask width
        pred_distri, pred_scores = torch.cat([xi.view(feats[0].shape[0], self.no, -1) for xi in feats], 2).split(
            (self.reg_max * 4, self.nc), 1#把每个尺度的 [B, no, H, W] 变成 [B, no, H*W]
        )

        # B, grids, ..
        pred_scores = pred_scores.permute(0, 2, 1).contiguous()#[B, N, nc]
        pred_distri = pred_distri.permute(0, 2, 1).contiguous()#[B, N, 4*reg_max]N:anchor数
        pred_masks = pred_masks.permute(0, 2, 1).contiguous()#[B, N, 32]

        dtype = pred_scores.dtype
        imgsz = torch.tensor(feats[0].shape[2:], device=self.device, dtype=dtype) * self.stride[0]  # image size (h,w)
        anchor_points, stride_tensor = make_anchors(feats, self.stride, 0.5)#多个特征图上的每个 cell 位置映射成原图中对应的位置（以中心点坐标形式），用于后续 bbox 解码（bbox_decode）
        ...
        try:
            ...
            targets = torch.cat((batch_idx, batch["cls"].view(-1, 1), batch["bboxes"]), 1)
            targets = self.preprocess(targets.to(self.device), batch_size, scale_tensor=imgsz[[1, 0, 1, 0]])
            #targets:(batch_size, max_objects_per_image, 5),5:[class_id, x_min, y_min, x_max, y_max]
            gt_labels, gt_bboxes = targets.split((1, 4), 2)  # cls, xyxy
            mask_gt = gt_bboxes.sum(2, keepdim=True).gt_(0.0)
        except RuntimeError as e:
            raise TypeError(
    ...
    def new_unsup_loss(self,preds,batch):
        ...
        # WARNING: lines below prevent Multi-GPU DDP 'unused gradient' PyTorch errors, do not remove
            ...
            loss[1] += (proto * 0).sum() + (pred_masks * 0).sum()  # inf sums may lead to nan loss
        #print(f"fg_mask is:{fg_mask.sum()}")
    ...
    def unsup_loss(self, preds, pseudo_label):
        ...
        #调整伪标签格式
        # resize_shape=imgsz[[1, 0, 1, 0]]
        # resize_shape = imgsz
        # ori_shape = torch.tensor([369,496],device=self.device,dtype=dtype)
        # scale_tensor = (resize_shape/ori_shape)[[1,0,1,0]]
        # gt_bboxes, gt_labels, mask_gt = self.convert_pseudo_labels(pseudo_label, batch_size, scale_tensor)

        gt_bboxes, gt_labels, mask_gt = self.convert_pseudo_labels(pseudo_label, batch_size)

        #ps_label(List):shape[bs,Tensor((x,6),(x,480,640))]
        ...
        target_scores_sum = max(target_scores.sum(), 1)
        unsup_bce_loss = self.bce(pred_scores, target_scores.to(dtype)).sum() / target_scores_sum  # BCE
        iou_loss = 0
        df_loss = 0
        if fg_mask.sum():
            # Bbox loss
            iou_loss, df_loss = self.bbox_loss(
                pred_distri,
                pred_bboxes,
                anchor_points,
                target_bboxes / stride_tensor,
        ...
        return (unsup_bce_loss*0.1 + iou_loss*0.45 + df_loss*0.45) * batch_size
        

    def convert_pseudo_labels(self, pseudo_labels, batch_size): 
        device = self.device       
    
        # 创建batch索引 (与原始代码中的targets格式一致)
        n_max_boxes = 0
        # batch_idx = []
        cls = []
        for i in range(batch_size):
            num_boxes = pseudo_labels[i][0].shape[0]
            n_max_boxes = max(n_max_boxes, num_boxes)
            # batch_idx.append(torch.full((num_boxes, 1), i, device=device))
        # batch_idx = torch.cat(batch_idx,dim=0)
        
        for i in range(batch_size):
            tmp_cls = pseudo_labels[i][0]
            x = tmp_cls.shape[0]
            box = tmp_cls[:,:4]
            tmp = torch.zeros((n_max_boxes,4),dtype=tmp_cls.dtype)
            valid_len = min(x, n_max_boxes)
            tmp[:valid_len,:] = box[:valid_len,:]
            cls.append(tmp)
        clses = torch.cat(cls, dim=0)#目前形状为bs*n_max_box,4;需要调整成bs,n_max_box,4
        gt_bboxes = (clses.reshape(batch_size,n_max_boxes,4)).to(device)
        #gt_bboxes[...,:4] = gt_bboxes[...,:4].mul_(scale_tensor)
        gt_labels = (torch.zeros(batch_size,n_max_boxes,1)).to(device)
        mask_gt = (gt_bboxes.sum(2, keepdim=True).gt_(0.0)).to(device)
    ...
    #     targets_list = []
    #     for i in range(batch_size):
    #         bbox_data, _ = pseudo_labels[i]
    #         if bbox_data.shape[0] > 0:
    #             # 构造 [batch_idx, cls, bbox] 格式
    #             cls = bbox_data[:, 5].view(-1, 1)
    #             bbox = bbox_data[:, :4]
    ...
    # # 与原始preprocess类似的逻辑
    #     if targets.shape[0] == 0:
    #         out = torch.zeros(batch_size, n_max_boxes, 5, device=device)
    #     else:
    #         i = targets[:, 0]  # 图像索引
    #         _, counts = torch.unique(i, return_counts=True)
    #         max_count = min(counts.max().item(), n_max_boxes) if counts.numel() > 0 else 0
        
    #         out = torch.zeros(batch_size, n_max_boxes, 5, device=device)
        
    #         for j in range(batch_size):
    #             matches = i == j
    #             n = min(matches.sum().item(), n_max_boxes)
    #             if n > 0:
    #                 out[j, :n] = targets[matches, 1:][:n]
    
    # # 拆分结果 (与原始代码一致)
    #     gt_labels, gt_bboxes = out.split((1, 4), dim=2)
    #     mask_gt = gt_bboxes.sum(2, keepdim=True).gt_(0.0)
    
        #return gt_bboxes, gt_labels, mask_gt

    @staticmethod
    def single_mask_dice_loss(
        gt_mask: torch.Tensor, pred: torch.Tensor, proto: torch.Tensor, xyxy: torch.Tensor, area: torch.Tensor
    ) -> torch.Tensor:
        """
        Compute the instance segmentation loss for a single image.
        ...
        inter = (pred_crop * gt_crop).sum(dim=(1, 2))   # (n,)
        union = pred_crop.sum(dim=(1, 2)) + gt_crop.sum(dim=(1, 2))  # (n,)
        dice_loss = 1.0 - (2.0 * inter + eps) / (union + eps)

        
        return (dice_loss / area).sum()

    def calculate_unsup_segmentation_loss(
        self,
        fg_mask: torch.Tensor,
        masks: torch.Tensor,
        target_gt_idx: torch.Tensor,
        ...
        for i, single_i in enumerate(zip(fg_mask, target_gt_idx, pred_masks, proto, mxyxy, marea, masks)):
            ...
            if fg_mask_i.any():
                ...
                loss += self.single_mask_dice_loss(
                    gt_mask, pred_masks_i[fg_mask_i], proto_i, mxyxy_i[fg_mask_i], marea_i[fg_mask_i]
                )
